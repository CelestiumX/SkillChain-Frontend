...
import { BarChart3, FileText, Home, MessageSquare, Settings, ShoppingBag, Star, Users, Wallet } from "lucide-react"
  ...
  return (
    <div className="fixed top-16 bottom-0 left-0 w-64 hidden lg:block bg-deep-space/90 backdrop-blur-md border-r border-sky-blue/10 overflow-y-auto z-30">
      ...
      <div className="px-3 py-4">
        <h3 className="px-4 text-xs font-semibold text-muted-foreground uppercase tracking-wider">Favorites</h3>
        <div className="mt-2 space-y-1">
          <SidebarItem
            href="/dashboard/favorites/web-design"
            icon={<Star className="h-5 w-5" />}
            label="Web Design"
            active={pathname === "/dashboard/favorites/web-design"}
          />
          <SidebarItem
            href="/dashboard/favorites/mobile-dev"
            icon={<Star className="h-5 w-5" />}
            label="Mobile Development"
            active={pathname === "/dashboard/favorites/mobile-dev"}
          />
        </div>
      </div>
